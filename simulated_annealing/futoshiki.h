#ifndef FUTOSHIKI_H_1DQ6ZZHW
#define FUTOSHIKI_H_1DQ6ZZHW
...
class Futoshiki
{
    ...
    public:
        ...
        int n, int m, int i, int j);                     // Funtion to alter the state of the configuration.
        double compute_energy();                         // Function to return energy of configuration.
		int check_inequalities();
        int verify_board();

        Futoshiki(const Futoshiki& rhs);                 // Copy constructor.
...
/**
*  \brief       A function which prints the current configuration of the game board whenever
...
*
*/            
void Futoshiki::alter_configuration(std::mt19937_64 rand, int n, int m, int i, int j) {
	...
	data[n][m] = rand() % data[4][4];
	}
	else if ( (n == 1 && m == 4) ) {
	data[n][m] = rand() % data[1][3];
	}
	else {
	data[n][m] = dist(rand);
	}
...
//
	...
	data[i][j] = rand() % data[4][3];
	}
	else if ( (i == 3 && j == 4) ) {
	data[i][j] = rand() % data[4][4];
	}
	else if ( (i == 1 && j == 4) ) {
	data[i][j] = rand() % data[1][3];
	}
	else {
	data[i][j] = dist(rand);
	}
...
double Futoshiki::compute_energy() {
	...
	constexpr double reward2 = 7.5;   	//< If the configuration has satisfied a complete row or column.
    constexpr double reward3 = 10;    	//< If the configuration has satisfied an inequality.
	...
	*************  REWARD 2 ****************
	...
	/* If either the row or columns product sum is equal to 120 then we know that
 ...
 *  \param       none . Uses the private variable data from the class to access current
 *               configuration's elements.
 *  \return      1 . 0 . The program will return 0 if the check is succesful and 1 otherwise.
 *
 */
...
#endif  /* end of include guard: FUTOSHIKI_H_1DQ6ZZHW */          
