/**
 * \file      	   futoshiki.cc
 * \brief          The following program is part of TCD's CS7IS2 course in Artifical Intelligence project. It uses
...
int main() {
    ...
    A.initial_configuration();				    //< Initialize beginning configuration.
	...
	/* Create a new object B of class Futoshiki (will be a copy of A). */
    ...
    std::cout << std::setw(60) << std::setfill('-') << '\n' << std::setfill(' ');
        ...
        while ( T >= 0.25 ) {
			...
			int &i = v[dist(seeded_engine)];
			int &j = v[dist(seeded_engine)];
			...
			B.alter_configuration(seeded_engine, m, n, i, j);
            ...
            if ( count % 100000 == 0 || count == 1) {
	          std::cout << std::setw(3) << count << "\t\t\t" <<
  	          std::setprecision(8) <<  T << "\t\t" << std::setprecision(8) <<
              old_energy << "\n";
                }
